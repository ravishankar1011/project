Feature: Payment Service CustomerProfile, EndCustomerProfile Profile Account for provider DBS
  ...
  Scenario Outline: Create CustomerProfile Account from request origin as CASH_SERVICE and on behalf of CUSTOMER and verify it's created successfully
    ...
    Given I create account for CustomerProfile with customerProfileId <customer_profile_identifier> from <request_origin> on behalf of <on_behalf_of> with provider DBS Bank Ltd and expect the header status <status_code>
      | identifier | currency   | country   | metadata   | on_behalf_of   |
      | AccountId  | <currency> | <country> | <metadata> | <on_behalf_of> |

    Then I wait until max time to verify the payment account AccountId from <request_origin> on behalf of <on_behalf_of> status as ACCOUNT_CREATED with provider DBS Bank Ltd for customerProfileId PSCPID1
    ...
    Then I verify CustomerProfile with id <customer_profile_identifier> has payment account AccountId from <request_origin> on behalf of <on_behalf_of> exists with provider <bank_name> with values
      ...
      | AccountId  | DBS-SG      | SGD      | SGP     | ACCOUNT_CREATED | <metadata> | <on_behalf_of> |

    Examples:
      | customer_profile_identifier | currency | country | metadata                    | request_origin | on_behalf_of | bank_name    | country | status_code |
      | PSCPID1                     | SGD      | SGP     | {"key": "IntegrationTest1"} | CASH_SERVICE   | CUSTOMER     | DBS Bank Ltd | SGP     | 200         |
      | PSCPID1                     | SGD      | SGP     | {"key": "IntegrationTest2"} | CASH_SERVICE   | CUSTOMER     | DBS Bank Ltd | SGP     | 200         |
      | PSCPID1                     | SGD      | SGP     | {"key": "IntegrationTest3"} | CASH_SERVICE   | CUSTOMER     | DBS Bank Ltd | SGP     | 200         |
  ...
  Scenario Outline: Create EndCustomerProfile account from request origin as CASH_SERVICE and on behalf of CUSTOMER and verify it's created successfully
    ...
    Given I create account for EndCustomerProfile with id <end_customer_profile_identifier> for CustomerProfile <customer_profile_identifier> from <request_origin> on behalf of <on_behalf_of> with provider DBS Bank Ltd and expect the header status <status_code>
      | identifier | currency   | country   | metadata   | on_behalf_of   |
      | AccountId  | <currency> | <country> | <metadata> | <on_behalf_of> |
    ...
    Examples:
      ...
      | PSCPID1                     | ECPID1                          | CUSTOMER     | CASH_SERVICE   | SGD      | SGP     | {"key": "IntegrationTest1"} | DBS Bank Ltd | SGP     | 200         |
      | PSCPID1                     | ECPID1                          | CUSTOMER     | CASH_SERVICE   | SGD      | SGP     | {"key": "IntegrationTest2"} | DBS Bank Ltd | SGP     | 200         |
      | PSCPID1                     | ECPID1                          | CUSTOMER     | CASH_SERVICE   | SGD      | SGP     | {"key": "IntegrationTest3"} | DBS Bank Ltd | SGP     | 200         |
  ...
  Scenario Outline: Create CustomerProfile Account and verify from request origin as CASH_SERVICE and on behalf of CASH_SERVICE and verify it's failed
    ...
    Examples:
      ...
      | PSCPID1                     | SGD      | SGP     | {"key": "IntegrationTest1"} | CASH_SERVICE   | CASH_SERVICE | PTM_1102    |
  ...
  Scenario Outline: Create CustomerProfile Account and verify from request origin CASH_SERVICE and on behalf of CUSTOMER and passing incorrect data verify it's failed
    ...
    Examples:
      | customer_profile_identifier | currency | country | metadata                    | request_origin | on_behalf_of | status_code |
      | PSCPID1                     | USD      | SGP     | {"key": "IntegrationTest1"} | CASH_SERVICE   | CUSTOMER     | PTM_1002    |
  ...
  Scenario Outline: Create CustomerProfile Account and verify from request origin different from CASH_SERVICE and on behalf of CASH_SERVICE and verify it's failed
    ...
    Examples:
      | customer_profile_identifier | currency | country | metadata                    | request_origin | on_behalf_of | status_code |
      | PSCPID1                     | SGD      | SGP     | {"key": "IntegrationTest1"} | CARD           | CUSTOMER     | PTM_1303    |
      | PSCPID1                     | SGD      | SGP     | {"key": "IntegrationTest1"} | INVESTMENT     | CUSTOMER     | PTM_1303    |
  ...
  Scenario Outline: Create CustomerProfile Account on which customer profile is not onboarded on to the provider
    ...
    Then I create below Customer-Profile
      ...
      | PSCID2              | PSCPID2                     | SG     | Stark | hugo@atlas.com | 8989548747   |
    ...
    Then I verify Customer-Profile exist with values
      ...
      | PSCID2              | PSCPID2                     | SG     | Stark | hugo@atlas.com | 8989548747   | ACTIVE |
    ...
    Examples:
      | customer_profile_identifier | currency | country | metadata                    | request_origin | on_behalf_of | status_code |
      | PSCPID2                     | SGD      | SGP     | {"key": "IntegrationTest1"} | CASH_SERVICE   | CUSTOMER     | E9702       |
