Feature: Check Limits for Internal Account Transfer out for the PLUS account

  Background: Create 2 HUGOBANK PLUS accounts
    ...
    When I initiate progress onboarding for the user UID2 to upgrade the account to L2 and expect an onboarding status of INITIATED

    Then I initiate the progress onboarding journey HUGOBANK_VERIFY_INCOME within the INCOME_VERIFICATION_STEP for the user UID2 and expect a status of JOURNEY_INITIATED

    Then I upload document1 for salaryDetails for the HUGOBANK_VERIFY_INCOME journey for user UID2 as the fundsProof

    Then I upload document2 for salaryDetails for the HUGOBANK_VERIFY_INCOME journey for user UID2 as the fundsProof

    Then I upload document3 for salaryDetails for the HUGOBANK_VERIFY_INCOME journey for user UID2 as the fundsProof

    Then I upload document1 for salaryDetails for the HUGOBANK_VERIFY_INCOME journey for user UID2 as the employmentProof

    Then I upload document2 for salaryDetails for the HUGOBANK_VERIFY_INCOME journey for user UID2 as the employmentProof

    Then I upload document3 for salaryDetails for the HUGOBANK_VERIFY_INCOME journey for user UID2 as the employmentProof

    Then I process the progress onboarding journey HUGOBANK_VERIFY_INCOME within the INCOME_VERIFICATION_STEP for user UID2, and expect a status JOURNEY_HOLD

    Then I update the HUGOBANK_VERIFY_INCOME journey status within the INCOME_VERIFICATION_STEP as ACCEPT if operator action status is OPERATOR_ACTION_REQUIRED for user UID2

    Then I submit the progress onboarding for user UID2, the onboarding status should be IN_PROGRESS and the account level should be L2

    Given I update the onboarding status as ACCEPT if operator action status is OPERATOR_ACTION_REQUIRED for user UID2 to upgrade the account to level L2

    And I check progress onboarding status for the user UID2, the onboardingStatus status should be COMPLETED

    And I get user details for user UID2 and the user profile status should be PROFILE_ACTIVE

    And I check the user details to confirm if user UID2 is L2 and the user profile status should be PROFILE_ACTIVE

    # add payee
    Given I initiate the initial user authorisation to ADD_PAYEE for user UID1 and expect a status of USER_AUTHORISATION_SUCCESS
  ...
  Scenario: I edit the Internal Transfer out Limits - send more than set limit and send exactly set limit amount
    ...
    Given I edit the Internal Transfer out limits for user UID1 to 100000 PKR and expect a status code of 200

    Then I check if the hard limits are updated to 100000 for user UID1
    ...
    Then I transfer 100010 PKR from user UID1 to user UID2 with no reference and valid payee id and expect a status code of 200
    ...
    Then I transfer 100000 PKR from user UID1 to user UID2 with no reference and valid payee id and expect a status code of 200

    And I check the balance of the wallet with product code CASH_WALLET_CURRENT for user UID1 and the balance should be 100000 PKR exact

    And I check the balance of the wallet with product code CASH_WALLET_CURRENT for user UID2 and the balance should be 100000 PKR exact
  ...
  Scenario: Create a schedule for Internal Payee payments
    ...
    Then I add the wallet with product code CASH_WALLET_CURRENT of user UID2 to user UID1 as payee PID1 with valid_swift_bic and expect a status code of 200 and a status of BENEFICIARY_STATUS_CREATED
