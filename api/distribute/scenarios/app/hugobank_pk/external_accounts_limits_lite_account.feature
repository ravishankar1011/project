Feature: Check Limits for External Account Transfer out for the lite account

  Background: Create a HUGOBANK LITE account to test limits scenarios
  ...
  Scenario: I edit the External Accounts Transfer out Limits - send more than set limit and send exactly set limit amount
    ...
    Given I edit the External Transfer out limits for user UID1 to 10000 PKR and expect a status code of 200
  ...
  Scenario: I add the external Bank - Test Bank Raast - 1Link to check the 1Link+Raast payments
    ...
    Then I add the user UID2 as External Payee PID1 for the user UID1 with External Account Number in the TTest Bank Raast - 1Link and expect a status of BENEFICIARY_STATUS_PENDING
    ...
    And I check the balance of the wallet with product code CASH_WALLET_CURRENT for user UID1 and the balance should be 40000 PKR exac
  ...
  Scenario: Create a schedule for external payments
    ...
    Then I add the user UID2 as External Payee PID1 for the user UID1 with External Account Number in the External Bank and expect a status of BENEFICIARY_STATUS_PENDING
