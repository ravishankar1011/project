Feature: Check Daily Receiving Limits for a lite user

  Background: Create a HUGOBANK LITE accounts to test limits scenarios
  ...
  Scenario: I edit the User's Receiving Limits - receive more than set limit and receive exactly set limit amount
    ...
    Given I edit the External Credit limits for user UID1 to 10000 PKR and expect a status code of 200
    ...
    Then I deposit 10100 PKR which is more than daily receiving limit of into wallet with product code CASH_WALLET_CURRENT to check limits for user UID1 and expect a status of 200
  ...
  Scenario: check the default limits (50000 PKR)

    Then I get the user cash wallets for the user UID1 and expect the account status of CASH_ACCOUNT_STATUS_CREATED

    # receive - above default limit (50000 PKR)
    Then I deposit 50010 PKR which is more than daily receiving limit of into wallet with product code CASH_WALLET_CURRENT to check limits for user UID1 and expect a status of 200

    And I check the balance of the wallet with product code CASH_WALLET_CURRENT for user UID1 and the balance should be 0 PKR exact

    # receive - exactly default limit (50000 PKR)
