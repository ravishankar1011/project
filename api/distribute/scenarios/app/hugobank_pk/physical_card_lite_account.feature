Feature: Cards Integration tests for LITE account
  ...
  Scenario: Physical PAYPAK Card transactions with default limits for Channel (ATM)
  ...
  Scenario: Physical PAYPAK Card transactions with updated limits for all channels
    ...
    And I update the Paypak Physical Card limit of channel E_COMMERCE to 10000 PKR for user UID1 and expect a status code of 200

    Then I check if the Paypak Physical Card limit for the E_COMMERCE channel is set to 10000 PKR for user UID1
    ...
    And I update the Paypak Physical Card limit of channel E_COMMERCE to 20000 PKR for user UID1 and expect a status code of 200

    Then I check if the Paypak Physical Card limit for the E_COMMERCE channel is set to 20000 PKR for user UID1

    Given I create below transaction for PHYSICAL card for user profile id and expect a status code of 200
      | user_profile_identifier | transaction_permutation_name | billing_amount | is_foreign_txn | channel    |
      | UID1                    | AUTH_CLEAR                   | 10000          | false          | E_COMMERCE |

    And I check the balance of the wallet with product code CASH_WALLET_CURRENT for user UID1 and the balance should be 30000 PKR exact
    ...
    And I update the Paypak Physical Card limit of channel ATM to 10000 PKR for user UID1 and expect a status code of 200

    Then I check if the Paypak Physical Card limit for the ATM channel is set to 10000 PKR for user UID1

    Given I create below transaction for PHYSICAL card for user profile id and expect a status code of 200
      | user_profile_identifier | transaction_permutation_name | billing_amount | is_foreign_txn | channel |
      | UID1                    | AUTH_CLEAR                   | 10500          | false          | ATM     |
    ...
    And I check the balance of the wallet with product code CASH_WALLET_CURRENT for user UID1 and the balance should be 20000 PKR exact
    ...
    And I update the Paypak Physical Card limit of channel ATM to 20000 PKR for user UID1 and expect a status code of 200

    Then I check if the Paypak Physical Card limit for the ATM channel is set to 20000 PKR for user UID1
    ...
    And I update the Paypak Physical Card limit of channel POS to 5000 PKR for user UID1 and expect a status code of 200

    Then I check if the Paypak Physical Card limit for the POS channel is set to 5000 PKR for user UID1

    Given I create below transaction for PHYSICAL card for user profile id and expect a status code of 200
      | user_profile_identifier | transaction_permutation_name | billing_amount | is_foreign_txn | channel |
      | UID1                    | AUTH_CLEAR                   | 5500           | false          | POS     |

    And I check the balance of the wallet with product code CASH_WALLET_CURRENT for user UID1 and the balance should be 10000 PKR exact

    Given I create below transaction for PHYSICAL card for user profile id and expect a status code of 200
      | user_profile_identifier | transaction_permutation_name | billing_amount | is_foreign_txn | channel |
      | UID1                    | AUTH_CLEAR                   | 5000           | false          | POS     |

    And I check the balance of the wallet with product code CASH_WALLET_CURRENT for user UID1 and the balance should be 5000 PKR exact
    ...
    And I update the Paypak Physical Card limit of channel POS to 10000 PKR for user UID1 and expect a status code of 200

    Then I check if the Paypak Physical Card limit for the POS channel is set to 10000 PKR for user UID1
  ...
  Scenario: Visa Physical Card transactions with default limits and updated limits for all channels
    ...
    And I update the Visa Physical Card limit of channel E_COMMERCE to 10000 PKR for user UID1 and expect a status code of 200

    Then I check if the Visa Physical Card limit for the E_COMMERCE channel is set to 10000 PKR for user UID1
    ...
    And I update the Visa Physical Card limit of channel E_COMMERCE to 20000 PKR for user UID1 and expect a status code of 200

    Then I check if the Visa Physical Card limit for the E_COMMERCE channel is set to 20000 PKR for user UID1
    ...
    And I update the Visa Physical Card limit of channel ATM to 10000 PKR for user UID1 and expect a status code of 200

    Then I check if the Visa Physical Card limit for the ATM channel is set to 10000 PKR for user UID1
    ...
    And I update the Visa Physical Card limit of channel ATM to 20000 PKR for user UID1 and expect a status code of 200

    Then I check if the Visa Physical Card limit for the ATM channel is set to 20000 PKR for user UID1
    ...
    And I update the Visa Physical Card limit of channel POS to 5000 PKR for user UID1 and expect a status code of 200

    Then I check if the Visa Physical Card limit for the POS channel is set to 5000 PKR for user UID1
    ...
    And I update the Visa Physical Card limit of channel POS to 10000 PKR for user UID1 and expect a status code of 200

    Then I check if the Visa Physical Card limit for the POS channel is set to 10000 PKR for user UID1
