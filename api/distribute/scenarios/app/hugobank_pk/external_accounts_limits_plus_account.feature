Feature: Check Limits for External Account Transfer out for the plus account
  ...
  Scenario: I edit the External Accounts Transfer out Limits - send more than set limit and send exactly set limit amount
    ...
    Then I add the user UID2 as External Payee PID1 for the user UID1 with External Account Number in the Bank AL-Habib and expect a status of BENEFICIARY_STATUS_PENDING
    ...
    Given I edit the External Transfer out limits for user UID1 to 100000 PKR and expect a status code of 200
    ...
    Then I transfer 101000 PKR from user UID1 to user UID2 with no reference and valid payee id and expect a status code of 200
    ...
    And I check the balance of the wallet with product code CASH_WALLET_CURRENT for user UID1 and the balance should have decreased to 100000 PKR plus the fees

  Scenario: I add the external Bank - Test Bank Raast to check the Raast payments
     # add payee
    Then I initiate the initial user authorisation to ADD_PAYEE for user UID1 and expect a status of USER_AUTHORISATION_SUCCESS
    ...
    Then I add the user UID2 as External Payee PID1 for the user UID1 with External Account Number in the Test Bank Raast and expect a status of BENEFICIARY_STATUS_PENDING
  ...
  Scenario: I add the external Bank - Test Bank 1Link to check the 1Link payments
     # add payee
    Then I initiate the initial user authorisation to ADD_PAYEE for user UID1 and expect a status of USER_AUTHORISATION_SUCCESS
    ...
    Then I add the user UID2 as External Payee PID1 for the user UID1 with External Account Number in the Test Bank 1Link and expect a status of BENEFICIARY_STATUS_PENDING
  ...
  Scenario: I add the external Bank - Test Bank Raast - 1Link to check the 1Link+Raast payments
     # add payee
    Then I initiate the initial user authorisation to ADD_PAYEE for user UID1 and expect a status of USER_AUTHORISATION_SUCCESS
    ...
    Then I add the user UID2 as External Payee PID1 for the user UID1 with External Account Number in the Test Bank Raast - 1Link and expect a status of BENEFICIARY_STATUS_PENDING
  ...
  Scenario: Create a schedule for external payments
    ...
    Then I create a schedule for Payee Payments PID1 and expect status 200
      ...
      | UID1                    | SID1                | DAILY     | SCHEDULE_PAYMENTS | CASH_WALLET_CURRENT |                 | 1           | 100000 |
    ...
    And I check the balance of the wallet with product code CASH_WALLET_CURRENT for user UID1 and the balance should be 100000 PKR approx
