Feature: Roundups scenarios for a LITE account

  Background: Create a HUGOBANK LITE accounts to test roundup scenarios
  ...
  Scenario: Create a new virtual card for a user, Check for Roundup updates, sweep to the main cash account
    ...
    And I check the balance of the wallet with product code CASH_WALLET_CURRENT for user UID1 and the balance should be 950 PKR exact
