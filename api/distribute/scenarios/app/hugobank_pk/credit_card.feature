...
#    -----------------------------We do not go through these steps to order credit card----------------------------------
#    Then I get the user cash wallets for the user UID1 and expect the account status of CASH_ACCOUNT_STATUS_CREATED
#    Then I get the user credit accounts for the user UID1 and expect the account status of CREDIT_ACCOUNT_STATUS_ACTIVE
...
#--------------------------------Activation and other steps------------------------------------------------------
    ...
    # have made some changes in py file for credit card
...
#------------------------I assume card is ordered and activated----------------------------
    Then I wait for card status as CARD_STATUS_ACTIVE to activate PHYSICAL card for user UID1
    And I store activated card details for user UID1
  ...
  Scenario: View Channel limits (POS,E_COMMERCE,ATM) -> Update Channel limits -> View Channel limits -> Get Channel limits history
#------------------------I assume card is ordered and activated----------------------------
...
#    #    ------------------user pays excess than bill amount---------------------------
  ...
  Scenario: User closes credit account using cash wallet
    ...
    Then I close credit account for user UID1 using settlement source CASH_WALLET_CURRENT and expect status code 200
...
#-------------------------------------------------------------------------------------------
  Scenario: Credit card transactions without activation
...
#    And I check the balance of the wallet with product code CASH_WALLET_CURRENT for user UID1 and the balance should be 60800 PKR approx
...
#    And I check the balance of the wallet with product code CASH_WALLET_CURRENT for user UID1 and the balance should be 60300 PKR approx
  ...
  Scenario: Credit card transactions after activation
